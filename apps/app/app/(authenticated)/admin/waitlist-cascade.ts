...
// cascadeWaitlistNotification (and apps/api's waitlist-timeout cron copy) —
// duplicated because apps/web, apps/app, and apps/api are separately
// deployed Next.js apps that can't import each other's route-local code;
// only packages/* is shared, and this mixes @repo/database + @repo/email
// with domain rules that don't belong in either provider-swappable package.
// Keep all three copies in sync if this logic changes.
